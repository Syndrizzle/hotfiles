...
function make_literal() {
  ...
  [[ "$caches" == "" ]] \
    && print '(box :class "control-center-empty-box" :height 740 :orientation "vertical" :space-evenly false (image :class "control-center-empty-banner" :valign "end" :vexpand true :path "$HOME/.config/eww/Control-Center/assets/clock.png" :image-width 200 :image-height 200) (label :vexpand true :valign "start" :wrap true :class "control-center-empty-label" :text "'$quote'"))' \
    || print "(scroll :height 940 :vscroll true (box :orientation 'vertical' :class 'control-center-scroll-box' :spacing 15 :space-evenly false $caches))"
}
