...
function notify_view() {
  notify-send -i xclipboard -a shot-clipboard -u low Screenshot "Copied to clipboard."
  xdg-open "$dir/$file"
  if [[ -e "$dir/$file" ]]; then
    notify-send -i "$dir/$file" -a shot -t 20000 -u low Screenshot "Screenshot Saved."
  else
    notify-send -i user-trash-full -a shot-deleted -u low Screenshot "Screenshot Deleted."
  fi
}
...
function countdown() {
  for sec in $(seq "$1" -1 1); do
    notify-send -i custom-clock-blue -a shot-icon -t 1000 Screenshot "Taking shot in $sec..."
    sleep 1
  done
}
...
function shotarea() {
  ...
  if [ "$bytes" ]; then
    ...
    notify-send -i custom-cancel -a shot-icon -u low Screenshot "Operation Cancelled."
  fi
}
